...
public class DBpediaService {
	...
	public static List<String> getListOfPublishedBooks(String name) {
		String query = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> PREFIX foaf: <http://xmlns.com/foaf/0.1/> PREFIX onto: <http://dbpedia.org/ontology/> SELECT ?string WHERE { ?uri rdf:type onto:Book . ?uri onto:author ?author .?author foaf:name '"
				+ name + "'@en .OPTIONAL {?uri rdfs:label ?string . FILTER (lang(?string) = 'en') }}";
	...
	public static List<String> getAbstract(String name) {
		...
		String query = "PREFIX xsd: <http://www.w3.org/2001/XMLSchema#> PREFIX foaf: <http://xmlns.com/foaf/0.1/> PREFIX dbo: <http://dbpedia.org/ontology/> PREFIX rdf: <"
				+ Constants.RDF
				+ "> PREFIX dbpedia: <http://dbpedia.org/resource/> SELECT ?abstract WHERE {?s dbo:abstract ?abstract . ?s foaf:name ?name .?s rdf:type dbo:Person .FILTER ( regex(str(?name), \""
				+ name + "\") && lang(?abstract) = 'en')}";
